= 001 - Project Generation for Apache Maven
...
Initializer for Apache Maven should help to generate a ready-to-use project structure for Apache Maven.
...
It is also implemented by an Apache Maven maintainer, and it is an active project.

==== Positive Consequences

* Toolbox is a library coming from the Apache Maven community.
...
Apache Maven has its own mechanism for generating new projects, which is realized as an Apache Maven plugin.
This plugin has common components that are responsible for the generation part.
The integration of the common components is tricky because they depend on Apache Maven Core infrastructure.
...
This Java DSL is demonstrated in own Apache Maven plugin / CLI tool, but it can be used as library.
...
- https://github.com/support-and-care/maven-initializer/issues/3#issuecomment-3324185347[Result of the Kickoff Meeting for initializer for Apache Maven]